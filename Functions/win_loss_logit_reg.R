...
win_los_reg <- function(
  ...
  reg_df <- simp_filt(
    ...
    mutate(
      ...
      # make a REGION a factor for regression
      mutate(
  ...
  # make correlation matrix name exluding name elements that do not effect obs.
  ...
  png(
    filename = str_c("Figures/corr_mat", corr_save_name, ".png"),
    width = 2000,
    height = 2000,
    pointsize = 44
    )
  ...
  st(
    cor_df,
    out = "csv",
    file = str_c("Data/Summary_Stats/summary_stats", corr_save_name, ".csv")
    )
  ...
  if(party_or_admin == "PARTY"){
    ...
    ggsave(
      ...
      stop(cat('Cannot tell which model to run. Did you set party_or_admin = "PARTY" or "ADMIN"?'))
    }
  ...
  write_csv(
    all_models_tidy,
    file = str_c("Data/Regression_Results/logit_model_table", save_name, ".csv")
  )
