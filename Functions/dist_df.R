...
# Works for properly (re)formatted appellate data, too.
...
# noBP_IMC = if TRUE, drop observations associated with BP oil spill in 
...
#     all districts.
# recode_S = Default is NULL. If TRUE, recode settlements as neither wins
#     nor losses. This is an optional argument; if it is not specified
#     the function simply treats it as a NULL  value. 
# recode_M = Default is NULL. If TRUE, recode mixed outcomes as neither
#     wins nor losses. This is an optional argument; if it is not specified
#     the function simply treats it as a NULL  value.
# appeals = Default is NULL. If TRUE, the denominator of the percent wins
#     wins calculation is calculated excluding missing outcomes - only an
#     an appropriate approach for appeals data.
...
build_dist_df <- function(df,
  ...
  # test input: if input df is specified as character string, use get() to
  # retrieve df of the same name from environment where function is working.
  # Otherwise, take df as object directly.
  ...
  df <- df %>%
    ...
    # if recode_S != NULL, recode settlements (DISP == 13) as neither
    # wins nor losses
    {
      if(!is.null(recode_S))
        mutate(
    ...
    # if recode_M != NULL, recode mixed outcomes (JUDGEMENT == 3) as neither
    # wins nor losses
    {
      if(!is.null(recode_M))
        mutate(
    ...
    {
      # for appeals data, missing outcomes cannot be used to calculate win rates
      if(!is.null(appeals ))
        mutate(
          .,
          pt_term = case_when(
            TERMDATE >= "1988-01-01" &
              # in district data, 0 or -8 JUDGMENT is missing
              # in appellate data, -8 JUDGMENT (OUTCOME) is missing
            JUDGMENT >= 1 ~ 1,
            TRUE ~ 0
            )
          )
        else
          # for non-appeals data, missing outcomes are part of wins and losses
          # e.g. settlements 
          mutate(
            .,
            pt_term = case_when(
    ...
    mutate(
      d_pt = n(),
      # percent of all cases of a given pt in a specific district
      d_pt_pct = round(d_pt/pt*100,4),
